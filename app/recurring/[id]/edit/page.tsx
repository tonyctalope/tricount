...
export default async function EditRecurringExpensePage({
  ...
  const expenseData = await prisma.recurringExpense.findFirst({
    where: {
      id,
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b sticky top-0 bg-background/95 backdrop-blur z-10">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center gap-3">
          ...
          <h1 className="text-base sm:text-lg font-semibold truncate">Modifier la récurrente</h1>
      ...
      <main className="container mx-auto px-4 py-4 sm:py-8">
        <Card className="max-w-2xl mx-auto">
          <CardContent className="p-4 sm:p-6">
            <RecurringExpenseForm users={couple.users} expense={expense} />
          </CardContent>
        </Card>
      </main>
    </div>
